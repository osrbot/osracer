...
class RaceMathTest(unittest.TestCase):
    ...
    def test_chassis_accepts_twist_and_publishes_ekf_covariance(self):
        package_root = self.source_package_root()
        repo_root = package_root.parent
        ...
        self.assertNotIn('ProjectVer', chassis_text)
        self.assertIn("self.create_subscription(\n            Twist, 'cmd_vel'", chassis_text)
        self.assertIn("math.atan(self.wheelbase * angular_z / linear_x)", chassis_text)
        ...
        self.assertIn('self.query_firmware_metadata()', chassis_text)
        self.assertIn('serial_conn.write(b"stream off\\n")', chassis_text)
        self.assertIn('serial_conn.write(b"fw version\\n")', chassis_text)
        self.assertIn('def parse_fw_version(line: str):', chassis_text)
        self.assertIn('if not line.startswith("FW_VERSION:"):', chassis_text)
        self.assertIn('Product', chassis_text)
        self.assertIn('Hardware', chassis_text)
        self.assertIn('Version', chassis_text)
        self.assertIn('BuildID', chassis_text)
        self.assertIn('Dirty', chassis_text)
        self.assertIn('Proto', chassis_text)
        self.assertIn('OSRCORE firmware: ', chassis_text)
        self.assertIn('self.write_serial("stream sync\\n")', chassis_text)
        self.assertNotIn('stream legacy', chassis_text)
