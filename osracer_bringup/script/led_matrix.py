...
class LedMatrixNode(Node):
    def __init__(self):
        ...
        # 使用 f-string 格式化日志消息
        self.get_logger().info(f"LedMatrix ready: port={self.port} baud={self.baud} topic={self.topic}")
    ...
    def open_port(self):
        ...
        try:
            ...
            # 使用 f-string 或 str() 转换
            self.get_logger().error(f"Serial open failed: {str(e)}")
            return False
    ...
    def cb_write(self, msg):
        ...
        try:
            ...
            self.get_logger().error(f"Unexpected error in cb_write: {str(e)}")
