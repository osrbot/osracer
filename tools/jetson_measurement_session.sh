...
sensor topic preflight and serial query latency into one output directory with a
measurement_session.json manifest.
...
python3 - "${MANIFEST}" "${OUTPUT_DIR}" "${SENSOR_STATUS}" "${SENSOR_SUMMARY}" "${SERIAL_STATUS}" "${SERIAL_REPORT}" <<'PY'
import datetime as dt
import json
import sys
from pathlib import Path

manifest, output_dir, sensor_status, sensor_summary, serial_status, serial_report = sys.argv[1:]
def existing(path):
    p = Path(path)
